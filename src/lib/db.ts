...
// biome-ignore lint/suspicious/noExplicitAny: sqlite driver instance
type SqliteInstance = any;

const migrations: Array<(db: SqliteInstance) => void> = [
	// v1: initial schema + FTS + drop legacy sync_meta
	(db) => {
		db.exec(`
...
function getUserVersion(sqlite: SqliteInstance): number {
	const row = sqlite.prepare("PRAGMA user_version").get();
	return row?.user_version ?? 0;
}

function runMigrations(sqlite: SqliteInstance) {
